...
from .configuration_mamba import MyMambaConfig
...
def decode(
    ...
    def should_stop(current_token, inference_params):
        if inference_params.seqlen_offset <= min_length:
            return False
    ...
    if min_new_tokens is not None:
        min_length = sequences[-1].shape[1] + min_new_tokens
    ...
    def generate(
        ...
        **kwargs
        ...
        if not do_sample:
            top_k = 1
        output = decode(
            input_ids, self, max_length=max_length, max_new_tokens=max_new_tokens, top_k=top_k, top_p=top_p, temperature=temperature, min_length=min_length, min_new_tokens=min_new_tokens,  **kwargs
        )
...
def create_block(
    ...
    if ssm_cfg is None:
        ssm_cfg = {}
    factory_kwargs = {"device": device, "dtype": dtype}
    mixer_cls = partial(MyMamba, layer_idx=layer_idx, **ssm_cfg, **factory_kwargs)
    norm_cls = partial(
...
class MyMambaPreTrainedModel(PreTrainedModel):
    config_class = MyMambaConfig
    base_model_prefix = "my_mamba"
    supports_gradient_checkpointing = True
    # TODO: add other relevant traits


class MyMambaModel(MyMambaPreTrainedModel):
    def __init__(
        self,
        config,
        device=None,
...
class MyMambaForCausalLM(MyMambaPreTrainedModel, GenerationMixin):
    ...
    def __init__(
        ...
        self.backbone = MyMambaModel(config, **factory_kwargs)
...
AutoConfig.register("my_mamba", MyMambaConfig)
AutoModelForCausalLM.register(MyMambaConfig, MyMambaForCausalLM)